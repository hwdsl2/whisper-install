...
> [!NOTE]
> 對於面向網際網路的部署，請使用[反向代理](#使用反向代理)新增 HTTPS。使用反向代理時，請在 `/etc/whisper/whisper.conf` 中設定 `WHISPER_LISTEN_ADDR=127.0.0.1`，以防止未加密連接埠被直接存取。
...
- 作業系統支援：Ubuntu 22.04+、Debian 12+、AlmaLinux/Rocky/CentOS 9+、RHEL 9+、Fedora
