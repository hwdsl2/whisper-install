...
def _env_int(name: str, default: int) -> int:
    ...
    is absent. systemd's ``EnvironmentFile=`` directive sets bare ``VAR=`` lines
    in /etc/whisper/whisper.conf as empty strings (not absent), so a user who
    clears a numeric option would otherwise hit ValueError in int(). Treat
    empty/whitespace values as "use the default".
    """
...
def _validate_openai_transcription_compat(
    ...
    if model_id == _UNSUPPORTED_DIARIZE_MODEL:
        raise HTTPException(
            status_code=400,
            detail=(
                "Model 'gpt-4o-transcribe-diarize' is not supported by this server."
            ),
        )
    if response_format.strip().lower() == "diarized_json":
        raise HTTPException(
            status_code=400,
            detail="response_format='diarized_json' is not supported by this server.",
        )
    if include:
        raise HTTPException(
            status_code=400,
    ...
    if known_speaker_names:
        raise HTTPException(
            status_code=400,
            detail="known_speaker_names is not supported by this server.",
        )
...
async def health():
    """Container liveness probe — used by the server to detect startup completion."""
...
if __name__ == "__main__":
    ...
    # Fall back to 0.0.0.0 when WHISPER_LISTEN_ADDR is unset or empty
    host = os.environ.get("WHISPER_LISTEN_ADDR", "0.0.0.0").strip() or "0.0.0.0"
    uvicorn.run(
        "api_server:app",
        host=host,
        port=port,
